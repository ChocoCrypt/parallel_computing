...
void MergeSortParalelo(vector<long int> &vec){
    ...
    cout<<"paralelo tiempo total :"<<t_stop-t_start <<endl;
...
int main(int argc , char **argv ){
    ...
    vector<long int> vec2 = readfile(tamvector);
    ...
    MergeSort(vec2);
    double t_stop2 = omp_get_wtime();
    cout<<"serial tiempo total   :"<<t_stop2-t_start2 <<endl;

    if(RealCheck(vec) && RealCheck(vec2)){
        cout<<"ambos están ordenados correctamente"<<endl;
    }
    else{
        cout<<"failed"<<endl;
    }
